...
按调用方指定阶段和 [分步协议](references/stages.md) 返回当前阶段 JSON；[output.md](references/output.md) 定义最终合并结构。调用方负责检查预测门槛；输入中的了解度评估仅作资料缺口参考，不代表所有维度都已了解。
...
运行方已按 [life-search.json](references/life-search.json) 全文扫描并冻结来源角色、时间线索与双链。依赖 [common-retrieval](../../common/retrieval/SKILL.md) 的工具与证据规则，按随调用加载的 [预测检索策略](references/retrieval.md) 选择入口、预算和覆盖范围。命中、目录分类、自我表达词和候选排序都不是人生判断。
