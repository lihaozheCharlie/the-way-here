...
const labels = {health:"健康",work:"工作",play:"玩乐",relationships:"关系",finance:"财务"};
...
export function LifeScenarios({report}:{report:LifePredictionReport}) {
  ...
  const [selected,setSelected] = useState(report.scenarios[0]?.id);
  ...
  return <>
    ...
    {scenario && <article className="prediction-detail life-scenario-detail" aria-label="所选生活情景">
      ...
      {step===0 && <section aria-label="五年生活">
        ...
        <details className="prediction-cross"><summary>五个方面会怎样变化</summary><dl className="life-dimensions">{scenario.dimensions.map(d=><div key={d.id}><dt>{labels[d.id]}</dt><dd><p>{d.future}</p><p>得到：{d.gain}</p><p>代价：{d.cost}</p></dd></div>)}</dl><p>居住与环境：{scenario.environment}</p></details>
      </section>}
    ...
    <details className="prediction-cross"><summary>对现在的你有哪些理解</summary><dl className="life-dimensions">{report.dimensions.map(d=><div key={d.id}><dt>{labels[d.id]}</dt><dd><p>{d.current}</p><p>想要的变化：{d.desired}</p>{d.constraints.map((c,i)=><p key={i}>{c}</p>)}</dd></div>)}</dl></details>
