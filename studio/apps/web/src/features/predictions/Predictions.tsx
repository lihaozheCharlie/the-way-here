...
export function Predictions({ revision, knowledgeBaseId }: { revision: number; knowledgeBaseId: string }) {
  ...
  return <>{(requestError || error) && <p role="alert">{requestError || error}</p>}<PredictionContent key={knowledgeBaseId} view={data} busy={busy} scan={() => void refresh("", true)} refresh={(thoughts,kind) => void refresh(thoughts,false,kind)} /><ContextualAgentDock revision={revision} context={{ scope: "看见未来", title: "聊聊未来的可能性", summary: data.report?.current || "从真实经历聊起，慢慢理解你想成为的人。", defaultMode: "read", launcherLabel: "聊聊这些可能性", compactLauncher: true, suggestions: ["这条预测里有哪些假设还需要我补充？", "我想说说自己想成为什么样的人。"] }} /></>;
}
