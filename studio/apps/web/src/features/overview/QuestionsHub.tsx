...
export function QuestionsHub({ revision }: { revision: number }) {
  ...
  const wallQuestions = filteredQuestions;
    ...
    <section className="questions-wall" aria-labelledby="questions-wall-title">
      <div className="questions-wall-head">
        ...
        <div className="questions-filters" aria-label="筛选话题">
          <button type="button" className={topicKind === "all" ? "active" : ""} aria-pressed={topicKind === "all"} onClick={() => { setTopicKind("all"); setTopicLimit(4); }}>全部</button>
          {availableKinds.map((kind) => <button type="button" key={kind} className={topicKind === kind ? "active" : ""} aria-pressed={topicKind === kind} title={conversationTopicKinds[kind].description} onClick={() => { setTopicKind(kind); setTopicLimit(4); }}>{conversationTopicKinds[kind].label}</button>)}
        </div>
      </div>
      ...
      {wallQuestions.length > topicCards.length ? <button type="button" className="questions-wall-more" onClick={() => setTopicLimit((current) => current + 4)}>再看 {Math.min(4, wallQuestions.length - topicCards.length)} 个话题 <Icon name="down" size={15} /></button> : null}
    </section>
