...
export function VoiceInput({ onConfirm }: { onConfirm: (text: string) => void }) {
  ...
  useEffect(() => { if (!recording) return; const timer = setTimeout(() => void stop(), RECORDING_LIMIT_MS); return () => clearTimeout(timer); }, [recording]);
  async function tidy() {
