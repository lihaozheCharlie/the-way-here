...
describe("stage relationships and events", () => {
  it("retains every event in the dense scrolling layout", () => {
    const html = renderToStaticMarkup(<StageFocus stage={stage(9)} revision={0} />);
  ...
  it("includes descriptions and sparse layout for two events and all three graph branches", () => {
    const html = renderToStaticMarkup(<StageFocus stage={stage(2)} revision={0} />);
  ...
  it("shows honest empty branches and an empty event state", () => {
    const html = renderToStaticMarkup(<StageFocus stage={{...stage(0), relatedPeople:[], relatedPlaces:[], relatedSystems:[]}} revision={0} />);
