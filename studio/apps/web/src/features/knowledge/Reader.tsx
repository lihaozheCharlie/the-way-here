...
import { DocumentOutline, EditableDocument, ReadOnlyDocument, documentHeadingPrefix, editableMarkdownDocument, markdownWithoutSourceRelations } from "../../shared/markdown";
...
export function Reader({ revision }: { revision: number }) {
  ...
  const documentBody = editableMarkdownDocument(page.renderedMarkdown, true).body;
  const outlineMarkdown = page.isSource ? markdownWithoutSourceRelations(documentBody) : documentBody;
  ...
  return (
    <div className="reader-layout">
      <article className="reader">
        ...
        {new URLSearchParams(location.search).has("detached") ? <ReadOnlyDocument showOutline={false} page={page} id={page.id} markdown={page.renderedMarkdown} /> : <EditableDocument page={page} onRenamed={(renamed) => navigate(pageHref(renamed.id), { replace: true, state: returnContext })} />}
      </article>
      <AuxPanel className="reader-evidence-panel" label="目录与证据" open={evidenceOpen} onToggle={() => setEvidenceOpen(value => !value)} railWidth={44}>
      <div className="evidence-panel">
        <DocumentOutline markdown={outlineMarkdown} inactive={!evidenceOpen} headingPrefix={documentHeadingPrefix(page.id)} />
        <h3>证据与关联</h3>
        ...
        {page.sources.length > 0 && <><b>来源</b><ul>{page.sources.map((source) => <li key={source}>{source}</li>)}</ul></>}
        {page.incomingLinks.length > 0 && <><b>被这些页面引用</b><ul>{page.incomingLinks.map((source) => <li key={source.id}><PageLink page={source} /></li>)}</ul></>}
      </div></AuxPanel>
      <PageAgentContext context={{ scope: `${page.isSource ? "原始材料" : "知识页面"} · ${graphCategoryNames[page.category] || page.category}`, title: page.title, pageId: page.id, summary: page.excerpt, defaultMode: page.isSource ? "read" : "write", suggestions: page.isSource ? ["这份原始记录可以支持哪些已有判断？请区分直接证据和推断。", "这份记录与哪些人生阶段、人物或反复循环有关？"] : ["我想补充一段与当前页面有关的新经历，请按现有规则更新。", "请检查当前页面是否缺少来源、反例、关联或状态追踪。"] }} />
    </div>
  );
}
