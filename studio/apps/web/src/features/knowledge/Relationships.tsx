...
export function Relationships({ revision }: { revision: number }) {
  ...
  return <div className="understanding-people-page">
    <header className="relationships-page-head"><div><h1>人与世界</h1><p>这里记录了出现在你世界里的人。模型已根据已有记录为他们分类，你可以随时调整。</p></div><div className="relationships-head-stat"><b>{data.totalPeople + data.roles.length}</b><span>个人与关系角色</span></div></header>
    <div className="relationships-workspace">
      <div className="relationships-graph-col">
        <section className="relationships-graph-card" aria-labelledby="relationship-web-title">
          ...
          <GraphViewport label="人物与关系角色网络" width={380} height={380}>
            ...
            <g className="relationships-center" role="button" tabIndex={0} aria-label="显示全部关系" onClick={() => changeRole("all")} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); changeRole("all"); } }}><circle cx="190" cy="190" r="26" /><text x="190" y="194">我</text></g>
