...
export function buildRunPrompt(mode: Exclude<WikiRun["mode"], "validate">, prompt: string, config: VaultConfig): string {
  ...
  const boundary = mode === "read"
    ...
    : mode === "write"
      ...
      : `这是可按需使用知识库的对话。先判断用户这轮的主要意图：寒暄、分享近况、表达感受或随意聊天时，直接自然回应；不要为判断是否写入而读取文件、检索旧记录或调用工具。不要把一句即时安排、感受或尚未展开的想法自动归档。只有用户请求查询或整理知识，或本轮提供了具体、证据充分且会实质修正既有长期理解的信息时，才读取 ${config.paths.agentInstructions}、路由注册表及最小必要 Skill，并按规则处理。写入还须确有明确受影响的页面和保留价值；拿不准时继续聊，不写入。用户明确要求只读时不要写入；规则、目录、批量修改或难以撤销的操作先确认范围。写入时保护原始笔记并完成质量检查。最终答复先回应用户想聊的事；不要主动汇报读了哪些规则、改了或没改哪些文件、哈希、质量门和工具限制。只有用户询问处理过程，或确有需要用户处理的写入失败、检查失败时，才简短说明；实际变更由界面详情展示。`;
  return `${boundary}\n\n${context}\n\n用户请求：\n${prompt}`;
}
