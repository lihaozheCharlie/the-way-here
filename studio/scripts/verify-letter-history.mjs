...
try {
  ...
  await page.getByRole('button', { name: '回到最新版本' }).click();
  const history=page.getByRole('button',{name:'历史版本',exact:true});
  await history.click();
  ...
  await page.screenshot({path:path.join(captures,'history-page.png')});
  expect(errors).toEqual([]);
  console.log('History selection, original entry, timestamps, active highlight, keyboard, dismissal and compact layout passed.');
} finally {
