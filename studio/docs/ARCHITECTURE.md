...
讲故事通过胶片条切换照片，每张保持独立输入，实时根据去除空白后的内容显示“写好了 / 还没写”圆点、可访问名称和不同提示语。此步没有人物编辑或逐问对话。空故事的提示语内提供“AI 帮你写”，生成完成直接回填目标照片，标明 AI 起草并保留编辑。切换查看其他照片不改变生成目标；运行期间禁用编辑，草稿先保存再发起任务。“收进理解”按照片顺序组合带序号与文件名的讲述，保留旧整段内容，限制总长六万字；允许部分照片留白，至少需要一段讲述。显式点击后确认整批内容，再按已有协议构建。
