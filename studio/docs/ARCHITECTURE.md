...
v5以一组scenarios替代两组独立职业/生活树。五维health/work/play/relationships/finance同时用于assessment v2与当前理解、未来状态；居住环境贯穿情景。evidence为去重账本，事实/愿望/计划/行动/结果/假设分型，情景通过ID引用。两个lenses只强调同一个情景的不同方面。三阶段描述五年路径；forks可为空，不再固定两个结果。概率支持null、5%刻度、independent可并存不求和及exclusive明确范围合计100%。证据充分度单独保留。

旧v4报告仍可阅读并标识旧版，新生成只接受v5；旧assessment版本要求重新扫描。输入thoughtKind为update或hypothesis，在HTTP边界验证并按知识库保存；假设不能作为当前事实证据，清空输入时移除冻结输入并省略可能携带旧想法的上一版比较上下文。全库检索覆盖五维及环境，不由Wiki构建调用。运行失败保留旧档案。模型语义验收与结构测试分开，不将类型检查冒充预测质量验证。
