...
`/predict-self` 通过带显式 `knowledgeBaseId` 的 `GET /api/predictions` 读取评分和档案，`POST /api/predictions/refresh` 返回 202 并异步预测。首次使用由 AI 扫描 Wiki 的了解度达到60分后开放；评分仅指证据覆盖，不是用户价值或预测准确率。唯一规则配置在 `knowledge-engine/skills/consume/predict-self/references/scoring.json`；`life-views` 负责指标求值与模型输出结构/引文校验，服务端负责调度，浏览器不读取 Vault。

预测仅由解锁后的预测自己页面经 POST /api/predictions/refresh 主动触发。Wiki 构建与预测调度完全解耦：write、auto、read、validate 均不调用预测；构建导致资料改变时仅反映在评分与旧档案的 stale 状态。预测自身是独立只读 Agent 会话。每次启动重新建立指定库索引，按内容与语义日期计算版本摘要。五领域分析和解释规则仅保存在共享 Skill。

每库串行调度，同一运行期间手动点击去重，没有构建自动触发或待续跑队列。输入冻结指定库的完整资料，在每库应用状态目录原子更新权限0600的 evidence.json（完整目录、pageId、版本与正文），同时附最多120,000字符的初读摘录。模型按共享Skill补查冻结全文，不能将初读截断误认为愿望不存在；引用校验覆盖完整冻结资料。完整快照只保留该库最近一次，恢复中的任务使用原冻结版本，模型不写文件；当前库切换不影响已启动任务。完成时重新计算输入版本，过期结果不发布，保留上一版并提示用户在预测页面手动重试；不自动启动下一轮。结果经过结构、枚举、数量边界及引用逐字核对后，通过原子重命名保存到本机应用状态目录下 workspace/knowledgeBase 隔离的 prediction state；包含当前及上一版、生成时间、证据版本、任务运行时引用与运行中的输入快照。生成物不进入 Git 或私人 Wiki，不作为下次预测的证据。

服务启动只恢复已经启动且未结束的会话，忽略旧版本遗留的 pending 自动续跑标记；缺失/失败会话显式标为失败；20 分钟超时结束并允许重试。刷新失败保留上一版。前端通过 prediction SSE 事件与运行中轮询展示进展，支持领域切换、二级分岔、逐步展示引文/反例/实验，以及现有 Agent 抽屉讨论。连接线使用独立固定高度区，终点与卡片行顶部对齐。图中可信度与因素强度不解释为人生概率。

验证采用临时匿名双库夹具，覆盖 60 分边界、缺/重复/未来证据、非法输出、跨库切换、重复请求、版本过期、重启恢复、构建不触发预测及页面 API 主动触发。没有运行真实私人预测作为开发测试。


预测输出将展示分组收敛为 work/工作、life/生活；其余分析维度在两组内综合。每组最多5条路径，每条2个不同结果，新增 dailyLife（具体日常场景）与 currentDetail（折叠背景）。验证器允许少量有依据的独立路线，拒绝超长树根（>24字）、超长标题（一级>14字/结果>20字）、缺失日常场景与重复标题；生成语义仍须检查是否是具体角色/生活形态。全页引言≤100字，避免内部标识占据产品首屏。旧 v1 档案保留在持久化存储供比较，但 API 不再作为可展示报告返回，提示重新生成。树根不裁切正文，完整背景由用户主动展开。

预测输出 v3 增加 branch.choice（45字符内的取舍）和 evidence.cue（40字符内事件线索）；结果摘要收紧至90字符，引用原文最多240字符、解释最多100字符。旧版本保留在本地档案但不以新契约呈现；重新预测生成新版本。工作/生活路线的实质差异与证据多样性由共享Skill判断，服务端验证字段和逐字引用，不复制人生判断。前端详情改为互斥三步，来源原话按需展开。

路线最多5条，优先4—5；有依据的独立分岔只有1—3条时按实际数量保留，禁止为数量把同一路线拆碎。没有可支持的路线时才整组说明缺口。行动最多3项以便排序和执行。

预测输出 v4 在路径和结果上增加 probability（0—100整数）及 probabilityReason（180字符内）。非空领域的路径合计100，每条路径的两个条件结果合计100；API校验范围、合计与解释字段。概率是模型在列出情景范围内的主观分配，confidence继续表示证据充分程度，两者不可换算。旧v3档案保留供比较，需重新预测后展示v4；页面不以了解程度或因素强度生成替代百分比。一级连线随路径比例改变粗细；因素仍用1—3级独立相对影响，显示方向SVG与细条刻度。

预测前由 life-views 按共享 Skill 的 references/life-search.json 做通用生活关键词召回，扫描绑定库全部原始文件与 Wiki 正文，不按日期、类别或初读预算裁切。evidence.json 的 lifeSearch 保存两侧扫描数、每组命中页面数及全部命中的 pageId/关键词/1起始行号，运行提示要求先审查命中和上下文、扩展检索再推演。搜索代码不判断个人愿望或给路线加权，语义判别仍由共享 Skill 负责；零关键词命中不代表没有相关生活意向。

了解程度的时间跨度与近期处境按有效引用的原始记录计量：优先使用 end/Start，缺失时识别原始日记文件名中的完整年月日，支持不补零、点/横线/斜线/逗号/中文分隔，严格校验日历有效性。未来日期、歧义引用与文件修改时间不参与时间评分；近期窗口按本地日历日衡量。此修复只更新确定性评分，不触发预测生成，也不改写日记日期元数据。


了解度扫描通过 POST /api/predictions/scan 主动启动独立只读任务，冻结当前库全部资料并阅读 Wiki。共享 understanding.md 指定四维0—4级语义标准，服务端校验完整维度和真实 Wiki 原文后按权重求和，达到60分开启预测。不以记录字数、数量、主动想法或历史跨度直接解锁。扫描结果按库持久化，展示日期、理由、引文与缺口；更新后提示重新扫描，失败保留上次评估，旧预测仍可阅读。扫描和预测互斥，均支持超时与恢复，均不由 Wiki 构建自动触发。主动想法最长4000字符，只参与预测、不参与了解度评分。

### 完整生活情景 v5

v5以一组scenarios替代两组独立职业/生活树。四维health/work/play/love（斯坦福《Designing Your Life》HWPL模型；health维度也涵盖收入安排、负债、缓冲与安全感）同时用于assessment v2与当前理解、未来状态；居住环境贯穿情景。evidence为去重账本，事实/愿望/计划/行动/结果/假设分型，情景通过ID引用。两个lenses只强调同一个情景的不同方面。三阶段描述五年路径；forks可为空，不再固定两个结果。概率支持null、5%刻度、independent可并存不求和及exclusive明确范围合计100%。证据充分度单独保留。

旧v4报告仍可阅读并标识旧版，新生成只接受v5；旧assessment版本要求重新扫描。输入thoughtKind为update或hypothesis，在HTTP边界验证并按知识库保存；假设不能作为当前事实证据，清空输入时移除冻结输入并省略可能携带旧想法的上一版比较上下文。全库检索覆盖四维及环境，不由Wiki构建调用。运行失败保留旧档案。模型语义验收与结构测试分开，不将类型检查冒充预测质量验证。

### 预测页面的渐进展示契约

五年情景沿用 version 5，解析器同时接受旧 health/work/play/love 和新 health/love/play/finance 四维集合，且报告与情景集合必须一致。新预测由服务端输出展示契约请求独立财务维度，知识判断继续由公共 Skill 负责。旧报告缺失财务时显示待补充，不将工作或健康内容重新标记为财务。整体节奏兼容原三阶段，并接受 months0_3/months3_12/years1_3/years3_5 四阶段。维度可提供 verdict、gainShare（可为 null）、gains/costs 和按行动/前提/警示分组的 notes；解析器校验枚举、范围和长度，缺失比重不补造数值。

页面默认选中第一条路径；点击当前节点回到现在状态，点击其他路径重置叙事步骤和维度展开状态。四维仅在叙事下方展示一次，每次只展开一个维度，注意点可单独展开。未标注维度的旧行动和工作行动保留在整体节奏中。
