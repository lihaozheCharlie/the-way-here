...
4. 页面内统一的上下文 Agent 抽屉创建任务；Run 固化 `knowledgeBaseId` 与配置快照。页面 Agent 的 `auto` 模式只聊天或只读查询，不写入。回复完成后独立的只读判断任务可给原回合附上 `wikiSuggestion`，仅在用户点击构建按钮后才启动后台写入任务；显式来源流程保留各自的只读或构建边界。
5. `AgentRuntimeRegistry` 按用户选择与配置解析 Codex 或 Pi，适配器把两者事件统一为消息、工具、审批、诊断和回合完成事件；审批请求停在界面等待用户决定。
6. `auto` 回合完成即结束；写入判断随后异步运行并更新原回合。用户点击构建按钮后，服务端从当前会话生成独立的写入 Run，界面继续保留聊天与输入。显式写入按知识库串行化，不同知识库可以独立运行；重启时会从对应运行时的会话记录恢复状态。Agent 文本增量通过轻量 `agent` SSE 事件直达对话，不逐字持久化或广播完整 Run；运行中的当前片段保存在服务端内存中供 SSE 重连补齐，最终回答仍以持久化 Run 为准。等待区只显示当前阶段，技术任务详情不在对话中展示。

普通 Wiki 检索由 Agent 先将问题改写成少量具体短语，再调用知识库绑定的检索入口。Pi 工具直接复用本轮 `WikiIndex`；Codex 使用同一检索整理逻辑的本地命令。结果只返回少量候选页面与片段，回答重要判断时再回读页面或来源。冻结预测继续使用它自己的证据工具，不接入当前库检索。
...
`RunCoordinator` 只依赖通用的 `AgentRuntimeProvider` seam，不包含 Codex 或 Pi 的协议分支。Codex 适配器继续复用 `codex-bridge`；Pi 适配器使用 `pi-agent-core` 驱动用户配置的模型，并提供限定范围的列表、搜索、读取和写入工具。只读与 `auto` 任务没有写工具；只有显式 `write` Run 可以写入。所有写入只允许落在当前 Run 固化的 Wiki 或来源目录，已有文件还要求读取时返回的 SHA-256，避免并发覆盖；规则、目录、批量重跑或难以撤销的操作仍需先确认范围。
...
Electron 的 `workspace.mjs` 负责选择持久化位置：显式开发覆盖 → 已保存的旧知识空间 → 自动创建的 userData/workspace。首次运行只注册随包的 `demo`，并将它设为默认知识库，不依赖源码仓库。旧版托管工作区中，只有路径和配置仍与自动创建的“我的知识库”完全一致且目录为空时，才移除该空库并切换默认值；含有记录或自定义配置的个人库保留。数据、配置和应用包分离。知识引擎由打包脚本从根目录权威资源收集，启动时阶段性替换应用管理副本，不复制领域判断到 Studio 代码。Python 与 PyYAML 随包分发，通过本地服务 PATH 提供给现有质量命令。已保存的外部空间失效时保留原设置并提供重新选择，禁止静默回退空库。升级只更新程序和管理资源；持久化 origin 仍由真实空间路径确定。
